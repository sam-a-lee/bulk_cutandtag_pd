...
#SBATCH --error=/scratch/prj/bcn_marzi_lab/analysis_cutandtag_pd_bulk/data_out/070_peaks/logs/078_feature_counts_celltype_%j.err
...
for ct in "${CELLTYPES[@]}"; do
  ...
  PEAK_BED="$(ls "${CONSENSUS_DIR}/${ct}_macs3_q_0_00001_peaks_clean.narrowPeak" 2>/dev/null | head -n1 || true)"
  if [ -z "${PEAK_BED}" ]; then
