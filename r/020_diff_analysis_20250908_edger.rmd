...
sex_plot <- ggplot(neuro_meta, 
  ...
  geom_bar(alpha = 0.8) +
  theme_bw() +
  scale_fill_manual(values = c("#e1235c","#002b5d"))
sex_plot
...
```{r sva_microglia}
# create model matrix 
micro_mod = model.matrix(~as.factor(group) + frip,
                         data = micro_meta)
# create null matrix
...
# recreate micro dgelist with clean counts
micro_dgelist <- DGEList(counts = micro_counts_clean, 
                         genes = micro_peak_coords_clean,
                         samples = micro_meta)
```


```{r micro_diff_edger_group}
# edger normalization
...
# plotMDS(micro_dgelist, 
#         method="bcv", 
#         col=as.numeric(micro_dgelist$samples$group))

# design matrix=
micro_design <- model.matrix(~ group + sex + pmi, 
...
# check MD plot - expect symmetry 
# plotMD(glmQLFTest(micro_fit, coef="grouppd"))

# get colnames for QLFTest
micro_cn <- colnames(micro_design)
micro_cn

# get pvalues via Ftest of QL fit
micro_qlf <- glmQLFTest(micro_fit,
                        coef = "grouppd")

# tob table
# don't sort! -> need to remerge with peak coords
micro_tt <- topTags(micro_qlf, 
                    n = Inf, 
                    sort.by = "none")$table

# compute normalized CPM and add mean CPM 
cpm_norm <- cpm(micro_dgelist, normalized.lib.sizes = TRUE)
mean_cpm <- rowMeans(cpm_norm, na.rm = TRUE)

# create results df
micro_res_df <- data.frame(
...
p <- micro_res_df$pvalue
lfc <- micro_res_df$logFC

# two-sided p -> |z|
z_abs <- qnorm(p / 2, lower.tail = FALSE)

# add direction from logFC
z <- sign(lfc) * z_abs

bc <- bacon(z)

# Inspect bias & inflation
bias(bc)       # estimated mean shift
inflation(bc)  # > 1 means inflated, < 1 deflated

z_cor  <- es(bc)       # bias + inflation corrected z-scores
p_cor  <- pval(bc)     # corresponding corrected p-values

tab$PValue.bacon <- p_cor
tab$FDR.bacon    <- p.adjust(p_cor, method = "BH")


bc <- bacon(micro_tt)
micro_tt$padj_bacon <- pval(bc) %>% p.adjust(method = "BH")
micro_tt$pvalue_bacon <- pval(bc)
```
...
qq_p_gg(micro_tt$PValue)
lambda_from_p(micro_tt$PValue)
```
...
micro_control_bw <- "/Users/samanthalee/Documents/bulk_cutandtag_pd/data_out/070_peaks/079_bigwigs/microglia/microglia_control_mean_CPM.bw"

micro_pd_bw <- "/Users/samanthalee/Documents/bulk_cutandtag_pd/data_out/070_peaks/079_bigwigs/microglia/microglia_pd_mean_CPM.bw"

micro_peaks <- "/Users/samanthalee/Documents/bulk_cutandtag_pd/data_out/070_peaks/079_bigwigs/microglia/consensus_microglia_macs3_q_0_00001_peaks.narrowPeak"

# import data
peaks <- import(micro_peaks)
ctl_bw <- import(micro_control_bw, format="BigWig")
dis_bw <- import(micro_pd_bw, format="BigWig")

# harmonize naming to ucsc
seqlevelsStyle(peaks) <- "NCBI"
seqlevelsStyle(ctl_bw) <- "NCBI"
seqlevelsStyle(dis_bw) <- "NCBI"

# Keep only standard chromosomes (drops KI/GL/KN scaffolds etc.)
peaks <- keepStandardChromosomes(peaks, pruning.mode = "coarse")
ctl_bw   <- keepStandardChromosomes(ctl_bw,   pruning.mode = "coarse")
dis_bw   <- keepStandardChromosomes(dis_bw,   pruning.mode = "coarse")

# use peak summits if your BED is peak centers; otherwise keep as is
peaks <- resize(peaks, fix="center", width=1)  

# window around peaks (e.g., +/- 3kb)
upstream  <- 3000
downstream<- 3000
target_width <- upstream + downstream + 1  # +1 to include center base
...
# check sample clustering
# expect control + pd to cluster separately
# plotMDS(oligo_dgelist, 
#         method="bcv", 
#         col=as.numeric(oligo_dgelist$samples$group))

# design matrix
oligo_design <- model.matrix(~ group, 
...
# check sample clustering
# expect control + pd to cluster separately
plotMDS(neuro_dgelist, 
        method="bcv", 
        col=as.numeric(neuro_dgelist$samples$group))
