...
# in your sample metadata 'samp'
micro_meta_svs$age_resid <- resid(lm(age ~ sex, data = micro_meta_svs))  # center/scale age first

# edgeR (preferred at tiny n)
library(splines)
design <- model.matrix(~ group + sex + ns(age_resid, df=2), data = samp)
...
# 1) Fit nuisance-only model
fit0 <- edgeR::glmFit(y, model.matrix(~ sex + ns(age,2), samples))
mu0  <- fit0$fitted.values

# 2) Refit with group and offset = log(mu0)
design1 <- model.matrix(~ group, samples)
y$offset <- log(mu0)  # add to any existing offsets from library sizes
fit1 <- edgeR::glmQLFit(y, design1, robust=TRUE)
res  <- edgeR::glmQLFTest(fit1, coef="groupPD")



# 1) Fit nuisance-only model
fit0 <- edgeR::glmFit(micro_dgelist, model.matrix(~ sex + age_resid,micro_meta_svs))
mu0  <- fit0$fitted.values

# 2) Refit with group and offset = log(mu0)
design1 <- model.matrix(~ group, micro_meta_svs)
micro_dgelist$offset <- log(mu0)  # add to any existing offsets from library sizes
fit1 <- edgeR::glmQLFit(micro_dgelist, design1, robust=TRUE)
res  <- edgeR::glmQLFTest(fit1, coef="grouppd")

micro_tt <- topTags(res, 
                    n = Inf, 
                    sort.by = "none")$table
```
...
# design matrix=
micro_design <- model.matrix(~ group + sv1, 
