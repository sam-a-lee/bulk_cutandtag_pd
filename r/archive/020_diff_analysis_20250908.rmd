...
```{r sva_microglia}
# create model matrix 
micro_mod = model.matrix(~as.factor(group) + pmi + sex,
                         data = micro_meta)
# create null matrix
...
# rename sv cols
colnames(micro_meta_svs)[10] <- c("sv") #"sv2", "sv3", "sv4", "sv5")
...
```{r frip_depth_corr}
micro_meta$lib_size <- colSums(micro_feat_clean[,-c(1:3)])


# 4. Examine correlation
cor_test <- cor.test(micro_meta$frip, micro_meta$lib_size, method = "spearman")
